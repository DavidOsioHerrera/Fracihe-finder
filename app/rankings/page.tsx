...
import { createClient } from '@/lib/supabase'           // ← Cambiado
...
export default function RankingsPage() {
  const supabase = createClient()                       // ← Agregado
    ...
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mb-4 text-sm text-zinc-600">
          Total de fragancias: <span className="font-semibold">{rankings.length}</span>
        </div>
                  ...
                  <div key={item.id} className="bg-white border border-zinc-200 rounded-3xl p-6 flex flex-col justify-between hover:shadow-md transition-all">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-100">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <ThumbsDown size={17} /> {item.dislikes || 0}
                        </button>
                      </div>
                      <div className="text-xs text-zinc-500">{item.gender}</div>
                    </div>
                  </div>
                )
              })}
